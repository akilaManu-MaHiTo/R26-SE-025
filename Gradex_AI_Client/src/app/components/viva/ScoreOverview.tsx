...
function ScoreCard({ icon, label, value, unit, hint, tone }: ScoreCardProps) {
  ...
  // gap-1 overrides Card's own flex gap-6, which is what actually spaced these
  // rows apart once the value row's margin was removed.
  return (
...
export function ScoreOverview({
  ...
  // Only the blocking case gets a caption. Frame-quality notes (enhanced /
  // soft / too small) are diagnostics, not something an examiner acts on, and
  // they are what made these cards tall — they stay in the exported report.
  const faceHint =
    ...
    <div className="space-y-3">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <ScoreCard
          ...
          unit="/ 100"
          // Hints are kept only where they say something the number cannot:
          // a blocked mark, or a preview that is not yet saved. The steady-state
          // caption restated what the card already shows, so it is omitted.
          hint={
            incomplete
