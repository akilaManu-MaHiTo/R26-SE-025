...
from config import ENGAGEMENT_LEVEL_SCORES, canonical_engagement_label
...
class EngagementRecognizer:
    def __init__(self, model_path: str = "models/engagement_cnn.pt") -> None:
        self.model_path = model_path
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.idx_to_class = {idx: class_name for idx, class_name in enumerate(CLASS_NAMES)}
    ...
    def _load_model(self) -> torch.nn.Module:
        ...
        if not model_file.exists():
            raise FileNotFoundError(f"Engagement model checkpoint not found: {model_file}")
    ...
    def predict(self, face_img: np.ndarray, logits: bool = False):
        ...
        if logits:
            ...
            output_scores = probabilities.detach().cpu().numpy()

        label = canonical_engagement_label(self.idx_to_class[pred_index])
        confidence = float(output_scores[pred_index])
        normalized_score = ENGAGEMENT_LEVEL_SCORES.get(label, 0.5)
        blended_score = (normalized_score * confidence) + (0.5 * (1.0 - confidence))
        return label, confidence, float(blended_score)


class EngagementDetector:
    def __init__(self, model_path: str = "models/engagement_cnn.pt", debug: bool = False) -> None:
        self.model_path = model_path
        self.model = EngagementRecognizer(model_path=model_path)
        self.idx_to_class = self.model.idx_to_class
        self.class_names = self.model.class_names
        self.labels = self.model.labels
        self._debug = debug

    def predict(self, face_bgr: np.ndarray) -> Tuple[str, float, float]:
        prediction = self._run_model(face_bgr)
        if self._debug:
            print(f"Engagement raw preds: {prediction}")
        label, confidence, score = self._parse_prediction(prediction)
    ...
    def _run_model(self, face_bgr: np.ndarray) -> Any:
        ...
        if method is not None:
            ...
            try:
                ...
                raise RuntimeError(f"Engagement model inference failed: {exc}") from exc
        ...
        raise RuntimeError("No compatible prediction method found for engagement model")

    def _parse_prediction(self, prediction: Any) -> Tuple[str, float, float]:
        labels = self._labels_from_model()

        if isinstance(prediction, str):
            label = canonical_engagement_label(prediction)
            return label, 1.0, ENGAGEMENT_LEVEL_SCORES.get(label, 0.5)

        if isinstance(prediction, dict):
            label = prediction.get("engagement") or prediction.get("label") or prediction.get("emotion")
            confidence = prediction.get("confidence") or prediction.get("score")
            if label is None:
                label, confidence = self._extract_from_scores(prediction, labels)
        ...
        if isinstance(prediction, tuple) and len(prediction) >= 1:
            label_candidate = prediction[0]
            confidence = None

            if len(prediction) >= 2:
                second = prediction[1]
                if isinstance(second, (dict, list, tuple, np.ndarray)):
                    parsed_label, parsed_conf = self._extract_from_scores(second, labels)
                    normalized_label = canonical_engagement_label(label_candidate if isinstance(label_candidate, str) else parsed_label)
                    return normalized_label, self._normalize_confidence(parsed_conf), ENGAGEMENT_LEVEL_SCORES.get(normalized_label, 0.5)
                confidence = second
        ...
        return "very_low", 0.0, ENGAGEMENT_LEVEL_SCORES["very_low"]

    def _labels_from_model(self) -> Optional[List[str]]:
        for attr_name in ("idx_to_class", "class_names", "labels"):
